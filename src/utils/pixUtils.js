// Tabela de polinômios CRC16 CCITT
const CRC16_TABLE = [
 ...
 * @returns {string} - CRC16 em hexadecimal maiúsculo com 4 caracteres
 */
export const calculateCRC16 = (str) => {
  ...
  for (let byte of bytes) {
    ...
    crc &= 0xFFFF; // Mantém apenas os 16 bits menos significativos
  }

  // Converte para hexadecimal com 4 caracteres
...
export const generatePixPayload = (data) => {
  const { chave, valor, beneficiario, cidade } = data;

  // Formata o valor com 2 casas decimais e remove pontos
  const valorAjustado = Number(valor) 
  const valorFormatado = valorAjustado.toFixed(2)

  // Formata a cidade em maiúsculas e remove acentos
  const cidadeFormatada = cidade.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');

  // Monta o payload seguindo o padrão EMV
  const payload = [
    '00020101021126500014', // Payload Format Indicator e Merchant Account Information
    'br.gov.bcb.pix', // PIX GUI
    ...
    '62290503***' // Point of Initiation Method
  ].join('');

  // Adiciona o CRC16 no final
  const payloadWithCRC = payload + '6304';
  const crc16 = calculateCRC16(payloadWithCRC);
